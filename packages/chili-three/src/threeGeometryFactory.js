...
import { LineType } from "chili-core";
import {
    AlwaysDepth,
    BufferAttribute,
    BufferGeometry,
...
export class ThreeGeometryFactory {
    static createVertexGeometry(data) {
    ...
    static createVertexMaterial(data) {
        return new PointsMaterial({
            size: data.size,
            sizeAttenuation: false,
        });
    }
    static createFaceGeometry(data, opacity) {
    ...
    static createMeshMaterial(opacity) {
    ...
    static setColor(buffer, data, material) {
    ...
    static createEdgeGeometry(data) {
    ...
    static createEdgeMaterial(data) {
    ...
    static createEdgeBufferGeometry(data) {
        let buff = new LineSegmentsGeometry();
        buff.setPositions(data.position);
        buff.computeBoundingBox();
        return buff;
    }
    static createVertexBufferGeometry(data) {
        let buff = new BufferGeometry();
        buff.setAttribute("position", new BufferAttribute(data.position, 3));
        buff.computeBoundingBox();
        return buff;
