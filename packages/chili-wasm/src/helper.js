...
export class OcctHelper {
    ...
    static convertFromMatrix(matrix) {
    ...
    static wrapShape(shape, id = Id.generate()) {
        if (shape.isNull()) {
            throw new Error("Shape is null");
        }
        switch (shape.shapeType()) {
    ...
    static wrapCurve(curve) {
        let isType = (type) => wasm.Transient.isInstance(curve, type);
        if (isType("Geom_Line")) return new OccLine(curve);
        else if (isType("Geom_Circle")) return new OccCircle(curve);
        else if (isType("Geom_Ellipse")) return new OccEllipse(curve);
        else if (isType("Geom_Hyperbola")) return new OccHyperbola(curve);
        else if (isType("Geom_Parabola")) return new OccParabola(curve);
        else if (isType("Geom_BezierCurve")) return new OccBezierCurve(curve);
        else if (isType("Geom_BSplineCurve")) return new OccBSplineCurve(curve);
        else if (isType("Geom_OffsetCurve")) return new OccOffsetCurve(curve);
        else if (isType("Geom_TrimmedCurve")) return new OccTrimmedCurve(curve);
        throw new Error("Unknown curve type: " + String(curve));
    }
    static getCurveType(curve) {
        let isType = (type) => wasm.Transient.isInstance(curve, type);
    ...
    static wrapSurface(surface) {
        let isType = (type) => wasm.Transient.isInstance(surface, type);
    ...
    static convertToContinuity(cni) {
        switch (cni) {
    ...
    static convertFromContinuity(continuity) {
        switch (continuity) {
            case Continuity.C0:
