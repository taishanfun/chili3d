...
import { concatTypedArrays, MathUtils } from "chili-core";
export class MeshUtils {
    static setFaceMeshData(data, other, matrix, offset) {
        if (!other) {
            return;
        }
    ...
    static setSurfaceMeshData(data, other, matrix, offset, materialMap) {
    ...
    static combineFaceMeshData(data, other, matrix) {
        if (!other) {
            return;
        }
        data.range = data.range.concat(
    ...
    static concatFloat32Array(arr1, arr2) {
        let arr = new Float32Array(arr1.length + arr2.length);
        arr.set(arr1);
        arr.set(arr2, arr1.length);
        return arr;
    }
    static setEdgeMeshData(data, other, matrix, offset) {
        if (!other) {
            return;
        }
    ...
    static combineEdgeMeshData(data, other, matrix) {
        if (!other) {
            return;
        }
    ...
    static mergeFaceMesh(mesh, materialMap) {
        if (materialMap.length === 0) {
            return mesh;
        }
        const materialIndexMap = new Map(materialMap);
    ...
    static mergeFaceMeshWithMap(mesh, materialMap) {
        const result = {
    ...
    static mergeSameGroup(group, mesh, result, offset) {
    ...
    static subFaceOutlines(face, index) {
        const mesh = this.subFace(face, index);
        if (!mesh) return undefined;
        return this.faceOutline(mesh);
    }
    static addEdge(pointsMap, face, i, j) {
