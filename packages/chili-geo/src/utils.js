...
import { ICurve, Orientation, Precision, Result, ShapeType, XYZ } from "chili-core";
export class GeoUtils {
    static nearestPoint(wire, point) {
    ...
    static normal(shape) {
        if (shape.shapeType === ShapeType.Face) {
            return shape.normal(0, 0)[1];
        }
        if (shape.shapeType === ShapeType.Edge) {
            let curve = shape.curve;
            return this.curveNormal(curve);
        }
        return this.wireNormal(shape);
    }
    static intersects(edge, otherEdges) {
        let result = [];
